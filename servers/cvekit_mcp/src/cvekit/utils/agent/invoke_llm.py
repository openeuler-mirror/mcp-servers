...
def initial_agent(project: Project, api_key: str, debug_mode: bool, provider: str = "openai"):
    """
    ...
    base_url, model_name = _get_llm_config(provider)
    logger.info(f"Using LLM provider: {provider}, model: {model_name}, base_url: {base_url}")
    ...
    logger.debug(f"  参数: temperature=0.5, model={model_name}, api_key={'***' + api_key[-4:] if api_key else 'None'}, openai_api_base={base_url}, verbose=True")
    llm = ChatOpenAI(
        temperature=0.5,
        model=model_name,
        api_key=api_key,
